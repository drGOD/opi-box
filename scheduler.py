...
class GrowboxScheduler:
    ...
    Background thread that:
      - Enforces relay on/off schedules (checked every 30 s)
      - Triggers timelapse snapshots at the configured interval
    """

    def __init__(self, relays: dict, camera, notifier, config: dict):
        ...
        self._timelapse_counter: int = 0  # minutes elapsed since last timelapse
    ...
    def _loop(self) -> None:
        while self._running:
            ...
            time.sleep(20)  # check every 20 s — catches every minute reliably
